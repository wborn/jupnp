...
public class IpAddressUtils {
	...
	public static boolean isSameIpAddress(String ip1, String ip2)
		...
		if (ip1Parts.length != 4) {
			throw new IllegalArgumentException("IpAddress 1 " + ip1
					+ " is invalid!");
		}
		if (ip2Parts.length != 4) {
			throw new IllegalArgumentException("IpAddress 2 " + ip2
					+ " is invalid!");
		}
		for (int i = 0; i < ip1Parts.length; i++) {
			try {
				...
				throw new IllegalArgumentException("IpAddress is invalid ("
						+ ex.getMessage() + "!");
			}
		}
		return true;
